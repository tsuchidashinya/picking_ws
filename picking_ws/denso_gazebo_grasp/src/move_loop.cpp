...
void Callback(const std_msgs::String::ConstPtr& msg)
{
  ...
  std::cout << "callback:" << buf.data << std::endl;
...
int main(int argc, char** argv)
{
  ...
  while (ros::ok())
  {
    ...
    {
      std::cout << "wait:forward" << std::endl;
      ros::spinOnce();
      std::cout << move_permission << std::endl;
    }
    ...
    while (!move_permission)
    {
      std::cout << "wait:side" << std::endl;
      ros::spinOnce();
    }
    std::cout << "go:side" << std::endl;
    plan_and_execute(group, target, speed_scale);
