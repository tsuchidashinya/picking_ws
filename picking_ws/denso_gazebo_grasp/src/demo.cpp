...
geometry_msgs::Pose arm_pose_place(int loop)
{
  geometry_msgs::Pose initial_pose;
  ...
  initial_pose.position.y = 0.25 + 0.15 * loop;
...
geometry_msgs::Pose set_arm_pose(const geometry_msgs::TransformStamped &transform)
{
  ...
  set_pose.orientation.x = 1.0;
  set_pose.orientation.y = 0.0;
  ...
  set_pose.position.z = transform.transform.translation.z + grasp_offset;
...
geometry_msgs::Pose set_arm_approach(const geometry_msgs::Pose &pose)
{
  geometry_msgs::Pose set_pose;
  set_pose = pose;
  set_pose.position.z = set_pose.position.z + 0.1;
...
int main(int argc, char **argv)
{
  ...
  double timeout = 3.0;
  geometry_msgs::TransformStamped near_object_transform;
  int near_object_number;
  ...
  while (ros::ok())
  {
    ...
    while (true)
    {
      try
      {
        ...
        object_transform = tfBuffer.lookupTransform("base_link", object_name, ros::Time(0), ros::Duration(timeout));
      }
    ...
    {
      std::cout << "\n=========================================" << std::endl;
      ROS_INFO("Work Success !!");
      std::cout << "\n=========================================" << std::endl;
      break;
    }

    //decision target
    near_object_transform = object_transforms[0];
    near_object_number = 1;
    for (int i = 1; i < object_number - 1; i++)
    {
      if (fabs(near_object_transform.transform.translation.y) > fabs(object_transforms[i].transform.translation.y))
      {
        near_object_transform = object_transforms[i];
        near_object_number = i + 1;
      }
    }
    std::cout << "\n=========================================" << std::endl;
    ROS_INFO("Target object is object_%d !!", near_object_number);
    std::cout << "\n=========================================" << std::endl;
    sleep(2);
    // initial
    ...
    std::cout << "\n========================================="
              ...
              << "\n========================================="
              << std::endl;
    target = set_arm_pose(near_object_transform);
    ...
    std::cout << "\n===================================="
              ...
              << "\n===================================="
              << std::endl;
    target = arm_pose_place(loop_count);
    target_approach = set_arm_approach(target);
    while (!trajectory_end)
    {
      plan_and_execute(group, target_approach, speed_scale);
